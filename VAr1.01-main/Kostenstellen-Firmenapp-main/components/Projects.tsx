...
interface ProjectsProps {
  invoices: Invoice[];
  projects: Project[]; // Initial-Daten (z.B. vom Backend)
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'invoices' | 'budget' | 'files' | 'tasks' | 'diary'>(
    'overview'
  );

  // Guard gegen Division by 0
  const usagePercent =
  ...
  const handleUpdateBudget = () => {
    if (newBudgetTotal !== project.budgetTotal) {
      if (
        window.confirm(
          `Möchten Sie das Gesamtbudget wirklich von € ${project.budgetTotal.toLocaleString('de-DE')} auf € ${newBudgetTotal.toLocaleString(
            'de-DE'
          )} ändern?`
        )
      ) {
  ...
  const costByCategory: Record<string, number> = projectInvoices.reduce(
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 min-h-[600px] flex flex-col animate-fade-in relative">
      ...
      <div className="p-6 border-b border-slate-100 flex justify-between items-start">
        <div>
          <button
            onClick={onBack}
            className="text-sm text-slate-500 hover:text-brand-600 mb-2 flex items-center"
          >
          ...
          <div className="flex items-center text-slate-500 mt-1 space-x-4">
            <span className="flex items-center text-sm">
              <MapPin className="w-4 h-4 mr-1" /> {project.address}
            </span>
            <span className="flex items-center text-sm">
              <Users className="w-4 h-4 mr-1" /> {project.manager}
            </span>
          </div>
        </div>
        <div className="text-right flex flex-col items-end space-y-2">
          <span
            className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
              usagePercent > 90 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
            }`}
          >
          ...
          <div className="flex space-x-2">
            <button
              ...
              className="text-xs text-slate-600 flex items-center hover:text-brand-600 border border-slate-200 px-3 py-1.5 rounded bg-slate-50"
            >
              <Printer className="w-3 h-3 mr-1" /> Tagebuch drucken
            </button>
            <button
              onClick={handleDownloadProjectFile}
              className="text-xs text-slate-500 flex items-center hover:text-slate-800 border border-slate-200 px-2 py-1 rounded"
            >
      ...
      <div className="flex border-b border-slate-100 px-6 overflow-x-auto">
        {(['overview', 'invoices', 'budget', 'diary', 'tasks', 'files'] as const).map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`mr-8 py-4 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
              activeTab === tab
              ...
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <p
                  className={`text-xl font-bold ${
                    usagePercent > 90 ? 'text-red-600' : 'text-slate-900'
                  }`}
                >
                  € {project.budgetUsed.toLocaleString('de-DE')}
                </p>
              </div>
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                ...
                {projectInvoices.slice(0, 3).map(inv => (
                  <div key={inv.id} className="relative">
                    ...
                    <p className="text-slate-800 font-medium">
                      Rechnung {inv.supplier} (€ {inv.amount}) erfasst
                    </p>
                  </div>
                ))}
                <div className="relative">
          ...
          <div className="animate-fade-in">
            <div className="flex justify-between mb-4">
              ...
              <button
                ...
                className="text-sm bg-brand-600 text-white px-3 py-1.5 rounded-lg hover:bg-brand-700 flex items-center"
              >
            ...
            {projectTasks.length === 0 ? (
              <p className="text-slate-400 text-center py-8">
                Keine Aufgaben für dieses Projekt gefunden.
              </p>
            ) : (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex text-xs text-slate-500 mt-1 space-x-3">
                        <span className="flex items-center">
                          <Users className="w-3 h-3 mr-1" /> {task.assignedTo}
                        </span>
                        <span className="flex items-center">
                    ...
                    <button
                      ...
                      className={`p-2 transition-colors rounded-full ${
                        task.completed
                          ...
                          : 'text-slate-300 hover:text-green-600 hover:bg-slate-50'
                      }`}
                    >
                  ...
                  {isEditingBudget ? (
                    <input
                      ...
                      onChange={e => {
                        ...
                        setNewBudgetTotal(v === '' ? 0 : parseFloat(v));
                      }}
                    />
                  ) : (
                ...
                <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                  ...
                  <p
                    className={`font-bold text-2xl ${
                      project.budgetTotal - project.budgetUsed < 0
                        ? 'text-red-600'
                        : 'text-green-600'
                    }`}
                  >
                    € {(project.budgetTotal - project.budgetUsed).toLocaleString('de-DE')}
                  </p>
                </div>
              </div>
            </div>
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex space-x-4 text-sm text-slate-600">
                          <span className="flex items-center" title="Wetter">
                            <Sun className="w-4 h-4 mr-1 text-orange-400" /> {entry.weather}
                          </span>
                          <span className="flex items-center" title="Temperatur">
                            <CloudRain className="w-4 h-4 mr-1 text-blue-400" />{' '}
                            {entry.temperature}
                          </span>
                          <span className="flex items-center" title="Personal">
                            <Users className="w-4 h-4 mr-1 text-slate-400" />{' '}
                            {entry.staffCount} Pers.
                          </span>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden">
            <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
              ...
              <button onClick={() => setIsDiaryModalOpen(false)}>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setNewDiaryEntry({ ...newDiaryEntry, weather: e.target.value })
                    }
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border-slate-300"
                    placeholder="20°C"
                    value={newDiaryEntry.temperature}
                    onChange={e =>
                      setNewDiaryEntry({
                        ...newDiaryEntry,
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">
                  Personalstärke (Anzahl)
                </label>
                <input
                  type="number"
                  className="w-full rounded-lg border-slate-300"
                  value={newDiaryEntry.staffCount}
                  onChange={e =>
                    setNewDiaryEntry({
                      ...newDiaryEntry,
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e =>
                    setNewDiaryEntry({ ...newDiaryEntry, content: e.target.value })
                  }
                ></textarea>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-slate-400">
                    {newDiaryEntry.images?.length || 0} Bilder ausgewählt
                  </span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="relative w-full max-w-[210mm] bg-white shadow-2xl min-h-[297mm] flex flex-col p-12 text-slate-900 font-sans print:shadow-none">
            <button
              ...
              className="absolute top-4 right-4 bg-slate-900 text-white p-2 rounded-full hover:bg-slate-700 print:hidden shadow-lg"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            <div className="border-b-2 border-slate-800 pb-4 mb-8 flex justify-between items-end">
              ...
              <div className="text-right text-sm">
                ...
                <p>Druckdatum: {new Date().toLocaleDateString('de-DE')}</p>
              </div>
            </div>
              ...
              {!project.diaryEntries || project.diaryEntries.length === 0 ? (
                ...
                project.diaryEntries.map(entry => (
                  ...
                  >
                    ...
                    <p className="text-sm whitespace-pre-wrap mb-4 font-serif leading-relaxed">
                      {entry.content}
                    </p>
                    {entry.images && entry.images.length > 0 && (
                      <div className="grid grid-cols-4 gap-2 mt-2">
                        {entry.images.map((img, i) => (
                          <img
                            key={i}
                            src={img}
                            className="w-full h-24 object-cover border border-slate-300 rounded"
                            alt="Doku"
                          />
                        ))}
                      </div>
...
// ---------- PERSISTENTE PROJECTS-KOMPONENTE ----------

const LOCAL_STORAGE_KEY = 'nexgen-projects';
...
const Projects: React.FC<ProjectsProps> = ({
  ...
  // Eigenes State-Array für Projekte, das in localStorage persistiert wird
  const [projectList, setProjectList] = useState<Project[]>(() => {
    ...
    try {
      ...
      console.warn('Konnte Projekte aus localStorage nicht laden:', e);
    }
    return projects;
  });

  // Falls vom Backend initial Projekte kommen, aber noch nichts in localStorage ist
  useEffect(() => {
  ...
  const persistProjects = (list: Project[]) => {
    ...
    try {
      ...
      console.warn('Konnte Projekte nicht in localStorage schreiben:', e);
    }
  };
  ...
  const handleCreate = () => {
    if (!newProj.name || !newProj.budgetTotal || !newProj.costCenterCode)
      return alert('Bitte Name, Budget und KSt-Nummer angeben.');
  ...
  // Wrapper für Updates aus ProjectDetail, damit auch localStorage aktualisiert wird
  const handleUpdateProjectLocal = (updated: Project) => {
  ...
  if (selectedProject) {
    ...
    // Immer die aktuellste Version aus projectList verwenden
    const liveProject = projectList.find(p => p.id === selectedProject.id) || selectedProject;
    ...
    return (
      <ProjectDetail
        ...
        onUpdateProject={handleUpdateProjectLocal}
        companySettings={companySettings}
      />
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700 shadow-sm flex items-center"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projectList.map(project => {
          const usageRatio =
            project.budgetTotal > 0 ? project.budgetUsed / project.budgetTotal : 0;
          ...
          return (
            <div
              ...
              className="bg-white group rounded-xl shadow-sm border border-slate-200 overflow-hidden cursor-pointer hover:shadow-md transition-all"
            >
              <div className="h-32 w-full bg-slate-100 relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span className="px-2 py-1 rounded-md text-xs font-bold bg-white/90 backdrop-blur-sm shadow-sm text-green-600">
                    {project.status}
                  </span>
                </div>
              ...
              <div className="p-5">
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-xs font-medium mb-1">
                    ...
                    <span className={usageRatio > 0.9 ? 'text-red-600' : 'text-slate-900'}>
                      {usagePercent}%
                    </span>
                  </div>
                  <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
                        usageRatio > 0.9 ? 'bg-red-500' : 'bg-brand-500'
                      }`}
                      style={{ width: `${Math.min(usagePercent, 100)}%` }}
                    ></div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">
                  Kostenstellen-Nr. (KSt) *
                </label>
                <div className="relative">
                  <input
                    type="text"
                    className="w-full rounded-lg border-slate-300 pl-3"
                    placeholder="KST-P-101"
                    value={newProj.costCenterCode || ''}
                    onChange={e =>
                      setNewProj({ ...newProj, costCenterCode: e.target.value })
                    }
                  />
                </div>
                <p className="text-[10px] text-slate-400 mt-1">
                  Prefix &apos;KST-&apos; wird automatisch ergänzt
                </p>
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">
                ...
                <input
                  ...
                  onChange={e =>
                    setNewProj({ ...newProj, address: e.target.value })
                  }
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border-slate-300"
                    placeholder="0.00"
                    value={newProj.budgetTotal || ''}
                    onChange={e => {
                      const v = e.target.value;
                      setNewProj({
                        ...newProj,
                        budgetTotal: v === '' ? undefined : parseFloat(v)
                      });
                    }}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setNewProj({ ...newProj, manager: e.target.value })
                    }
                  />
                </div>
              </div>
              <button
                ...
                className="w-full bg-brand-600 text-white py-2 rounded-lg font-medium hover:bg-brand-700 mt-2"
              >
                Projekt erstellen
              </button>
            </div>
